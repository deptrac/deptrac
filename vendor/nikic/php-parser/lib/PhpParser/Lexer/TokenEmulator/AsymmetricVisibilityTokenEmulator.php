...
final class AsymmetricVisibilityTokenEmulator extends TokenEmulator
{
    ...
    {
        $map = [\T_PUBLIC => \DEPTRAC_INTERNAL\T_PUBLIC_SET, \T_PROTECTED => \DEPTRAC_INTERNAL\T_PROTECTED_SET, \T_PRIVATE => \DEPTRAC_INTERNAL\T_PRIVATE_SET];
    ...
    {
        $reverseMap = [\DEPTRAC_INTERNAL\T_PUBLIC_SET => \T_PUBLIC, \DEPTRAC_INTERNAL\T_PROTECTED_SET => \T_PROTECTED, \DEPTRAC_INTERNAL\T_PRIVATE_SET => \T_PRIVATE];
        for ($i = 0, $c = \count($tokens); $i < $c; ++$i) {
            $token = $tokens[$i];
